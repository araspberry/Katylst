    ...
    <main>
      <section className="bg-k-bg pt-24 pb-16 px-6">
        <div className="max-w-[1100px] mx-auto text-center">
          <p className="text-[10.5px] font-bold tracking-[2.5px] uppercase text-k-orange mb-4">Get In Touch</p>
          <h1 className="font-serif italic text-[clamp(40px,5vw,72px)] leading-[1.05] tracking-[-2px] text-k-dark mb-5">
            Let&apos;s build something<br />
            <span className="text-k-orange">that gets you leads.</span>
          </h1>
      ...
      <section className="py-20 px-6 bg-white">
        <div className="max-w-[1100px] mx-auto grid grid-cols-1 lg:grid-cols-[1fr_1.6fr] gap-16">
          <div>
            <h2 className="font-serif italic text-[28px] text-k-dark mb-8">Contact info</h2>
            {[
              ...
              <div key={label} className="flex items-start gap-4 mb-6">
                ...
                <div>
                  <div className="text-[11px] font-bold tracking-[1.5px] uppercase text-k-mid mb-0.5">{label}</div>
                  <div className="text-[15px] font-semibold text-k-dark">{value}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="bg-k-bg rounded-[20px] p-8 md:p-10">
            {status === 'done' ? (
              <div className="text-center py-12">
                ...
                <h3 className="font-serif italic text-[24px] text-k-dark mb-2">Message sent!</h3>
                <p className="text-k-mid">We&apos;ll be in touch within one business day.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-[11px] font-bold tracking-[1.5px] uppercase text-k-mid mb-2">Name *</label>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold tracking-[1.5px] uppercase text-k-mid mb-2">Email *</label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-[11px] font-bold tracking-[1.5px] uppercase text-k-mid mb-2">Phone</label>
                    <input value={form.phone} onChange={e => setForm(f => ({...f, phone: e.target.value}))}
                      className="w-full bg-white border border-k-border rounded-[10px] px-4 py-3 text-[14px] text-k-dark outline-none focus:border-k-orange transition-colors"
                      placeholder="(555) 000-0000" />
                  </div>
                  <div>
                    <label className="block text-[11px] font-bold tracking-[1.5px] uppercase text-k-mid mb-2">Service</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold tracking-[1.5px] uppercase text-k-mid mb-2">Message *</label>
